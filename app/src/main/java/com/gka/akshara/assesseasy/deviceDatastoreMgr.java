...
import java.util.UUID;
    ...
    /*    public  String makePlaceholders(int len) {
        ...
        public ArrayList<com.akshara.assessment.a3.TelemetryReport.pojoReportData> getAllStudentsForReports(String questionsetId, ArrayList<StudentTable> studentIds, boolean flag, String assessmenttype, boolean by) {
            ...
            Cursor cs1 = null;
            //if flag variable true then it online report else offline db report
            String tableName = "";
            if (flag) {
                tableName = "Assessment_Table";
            } else {
                tableName = "a3app_assessment_tbl";
            }
            ...
            for (com.akshara.assessment.a3.db.StudentTable sID : studentIds) {
                ...
                if (flag) {
                    if(by) {
                       ...
                       /* String[] names = { "5","14"}; // do whatever is needed first

                        String query = "SELECT id,id_assessment,id_child,id_questionset,score,datetime_start,datetime_submission,id_questionset FROM "+tableName+
                                 " WHERE id_child="+sID.getId()+ " AND name IN (" + makePlaceholders(names.length) + ")";

                        cs1= a3appdb.rawQuery(query,names);
                 */      cs1 = a3appdb.query(tableName, new String[]{"id", "id_assessment", "id_child",
                                "id_questionset", "score", "datetime_start", "datetime_submission", "id_questionset"}, "assessmenttype=? AND id_child=?", new String[]{
                                assessmenttype, sID.getId() + ""}, null, null, "datetime_start" + " DESC", "1");
                    }else {
                        cs1 = a3appdb.query(tableName, new String[]{"id", "id_assessment", "id_child",
                                "id_questionset", "score", "datetime_start", "datetime_submission", "id_questionset"}, "id_questionset=? AND id_child=?", new String[]{
                                questionsetId, sID.getId() + ""}, null, null, "datetime_start" + " DESC", "1");
                ...
                if (cs1!=null&&cs1.getCount() > 0) {
                    ...
                    while (cs1.moveToNext()) {
                        ...
                        String tablename2 = "";
                        Cursor assessmentDetailCursor = null;
                        if (flag) {
                            tablename2 = "Assessment_Detail_Table";
                            assessmentDetailCursor = a3appdb.query(tablename2, new String[]{"id", "id_assessment", "id_question",
                                    "pass"}, "id_assessment=?", new String[]{

                                    id_assessment + ""}, null, null, null);

                        } else {
                            tablename2 = "a3app_assessmentdetail_tbl";
                            assessmentDetailCursor = a3appdb.query(tablename2, new String[]{"id", "id_assessment", "id_question",
                        ...
                        if (assessmentDetailCursor != null && assessmentDetailCursor.getCount() > 0) {
                            ...
                            while (assessmentDetailCursor.moveToNext()) {
                                ...
                                if (flag) {
                                    pojoAssessmentDetail.setAnswer_given("K");
                                    pojoAssessmentDetail.setSynced(0);
                        ...
                        if(assessmentDetailCursor!=null)
                        {
                            assessmentDetailCursor.close();
                            System.gc();
                        }
                    }
        ...
        public boolean isStudentAssessmentTaken(ArrayList<String> questionsetId, long studentID, String assmenttype) {
            try {
                String tableName = "a3app_assessment_tbl";
                Cursor cs1 = null;
                for (String qsetId : questionsetId) {

                    cs1 = a3appdb.query(tableName, new String[]{"id", "id_assessment"}, "id_questionset=? AND id_child=?", new String[]{
                            qsetId, studentID + ""}, null, null, null);
                    if (cs1 != null && cs1.getCount() > 0) {
                        return true;
                    }
                }

                cs1 = a3appdb.query("Assessment_Table", new String[]{"id", "id_assessment"}, "assessmenttype=? AND id_child=?", new String[]{
                        assmenttype, studentID + ""}, null, null, null);
                if (cs1 != null && cs1.getCount() > 0) {
                    return true;
                }
            }catch (Exception e)
